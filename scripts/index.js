...
// Opens the modal (shows overlay + popup)
function openModal(modal) {
  modal.classList.add("popup_is-opened");
  // Attach the overlay click listener to the modal itself
  // This ensures clicks outside the popup trigger closing
  modal.addEventListener("click", overLapClosesModal);
  window.addEventListener("keydown", escKeyClosesModal);
}

// Closes the modal (hides overlay + popup)
function closeModal(modal) {
  ...
  modal.removeEventListener("click", overLapClosesModal);
  window.removeEventListener("keydown", escKeyClosesModal);
  // resets validation once the modal is closed
  resetForm(modal);
}

// Handles closing the modal when clicking on the overlay (background)
function overLapClosesModal(evt) {
...
function escKeyClosesModal(evt) {
  if (evt.key === "Escape") {
